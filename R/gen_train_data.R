...
for (i_ts in 1:length(data))
{
  ...
  #****log_pred_densities nrow+1 ****
  ## Calculate historical log predictive density
  log_pred_densities<-matrix(nrow = length(y) - history_burn - train_h +1, ncol = 2)
  ...
  for (t in (history_burn):(length(y) - train_h))
  {
    ...
    log_pred_densities[(t - history_burn +1), 1] <- sum(dnorm(y01[(t + 1):(t + train_h)], mean = ets_fore_mean,
                                                           sd = ets_fore_sd, log = TRUE))
    log_pred_densities[(t - history_burn +1), 2] <- sum(dnorm(y01[(t + 1):(t + train_h)], mean = arima_fore_mean,
  ...
  features_y <- matrix(nrow = length(y) - train_h - history_burn +1, ncol = 42)
...
save(feat, lpred_dens, data, model_conf, file="E:/time series/R code/feature-based-bayesian-model-averaging/data/historical_log_pred_features.RData")
